...
requestAnimationFrame(function gameLoop() {
  ...
  // Projectile.stuff.toggle.draw(context).move();
  towers.forEach(function(tower) {
    tower.draw(context);
  });
  // console.log(Projectile.stuff.x, Projectile.stuff.y);
  context.drawImage(img, Projectile.stuff.x, Projectile.stuff.y);
...
canvas.addEventListener('keyup', function(event) {
  ...
  // var projectiles = [];
  debugger;
  if (keyCode === 32 && Projectile.stuff.projectiles.length === 0) {
    Projectile.stuff.toggle.pause = true;
    Projectile.stuff.projectiles.push(new Projectile.projectile(cannonBarrel, this));
    Projectile.stuff.projectiles.forEach(function(projectile) {
      projectile.move(cannonBarrel, Projectile.stuff.toggle.y);
    });
  }
});
