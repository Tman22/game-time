...
function Collision() {
  ...
  //   var one = (ball.x < tower.x && tower.x < (ball.x+ball.width) && tower.y > ball.y && tower.y < (ball.y+ball.height));
  //   // var one = (ball.x < tower.x && tower.x < (ball.x+ball.width) && tower.y > ball.y && tower.y < (ball.y+ball.height));
  ...
  //   } else {
  //     return context.drawImage(img, Zeus.projectileCoord.x, Zeus.projectileCoord.y, 15, 15);
  //   }
  // })
}
...
canvas.addEventListener('keyup', function(event) {
  ...
  if (keyCode === 32 && Zeus.projectiles.length === 0) {
    ...
    Zeus.projectiles.forEach(function(projectile) {
      projectile.move(cannonBarrel, Zeus.toggle.y, Zeus, towers);
    });
  }
});
